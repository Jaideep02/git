...
class GUI(STYLE):
    ...
    def generate_sudoku_board(self):
        ...
        Board_frame.grid(row=0, column=2, pady=90, padx=10)
        ...
        for i in range(9):
            ...
            for j in range(9):
                # Space Between rows
                if (p+1) % 4 == 0 and p != 0:
                    l1 = tk.Frame(Board_frame, width = 12, height = 4, bg = "#c6c8df")
                    l1.grid(row = p, column = 0, columnspan = 11, sticky = "ew")
                    p += 1
                # Space Between columns
                if (q+1) % 4 == 0 and q != 0:
                    l2 = tk.Frame(Board_frame, width = 4, height = 12, bg = "#c6c8df")
                    l2.grid(row = 0, column = q, rowspan = 11, sticky = "ns")
                    q += 1
    ...
    def right_side_option_block(self):
        ...
        right_block_frame = tk.Frame(self._master,bg="#c6c8df")
        ...
        # |           SUDOKU             |
        # +------------------------------+
        title_frame = tk.Frame(right_block_frame,bg="#c6c8df")
        ...
        title.grid(padx=250, pady=15)

        title_frame.grid()
        ## {1-1}

        # {1-2}  All Button option [ New Game, Clear, Help]
        option_frame = tk.Frame(right_block_frame,bg="#c6c8df")
        game_label_frame = tk.Frame(right_block_frame,bg="#c6c8df")

        # {1-2-1} New Game LabelFrame = [ Easy, Hard ]
        # +------------------------------+
        # |          NEW Game            |
        # +------------------------------+
        game_label_frame = tk.LabelFrame(option_frame,text="  NEW GAME ", font = ("Ink free",15))
        self.Option_Frame_Add_Style(game_label_frame)

        easy_game = tk.Button(game_label_frame, text = "Easy", command= lambda : self.easy_hard_game_button_action(41))
        ...
        hard_game = tk.Button(game_label_frame, text = "Hard", command= lambda : self.easy_hard_game_button_action(56))
        ...
        game_label_frame.grid(row=0, column=0, pady=self.Option_Frame_pady)
        ## {1-2-1}
        ...
        # |            CLEAR             |
        # +------------------------------+
        clear_label_frame = tk.LabelFrame(option_frame, text="  CLEAR  ", font = ("Ink free",15))
        ...
        # {1-2-3} Help LabelFrame = [ Hint, solver ]
        # +------------------------------+
        # |             HELP             |
        # +------------------------------+
        help_label_frame = tk.LabelFrame(option_frame, text="HELP", font = ("Ink free",15))
        ...
        speed_game = tk.Button(help_label_frame, text = "Solver", command=lambda : self.speed_visual_solve_button_action(False))
